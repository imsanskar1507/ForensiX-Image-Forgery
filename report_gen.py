...
def clean_text(text):
    """Removes Emojis and non-latin characters that cause Network Errors."""
    if not text: return ""
    # Strip everything except basic alphanumeric and punctuation
...
def create_pdf_report(results_list, case_notes=""):
    ...
    for res in results_list:
        ...
        pdf.set_font("Courier", size=10)
        # Remove emojis from verdict for PDF compatibility
        v_clean = res['VERDICT'].replace("🚩 ", "").replace("🏳️ ", "")
        pdf.cell(0, 6, txt=f"VERDICT: {v_clean} | CONFIDENCE: {res['CONFIDENCE']:.2f}%", ln=True)
