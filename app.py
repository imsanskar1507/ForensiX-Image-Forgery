...
def generate_heatmap(original_img_bytes, ela_img):
    """Generates a forensic heatmap by blending ELA results with the source image."""
    nparr = np.frombuffer(original_img_bytes, np.uint8)
    original = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    original = cv2.cvtColor(original, cv2.COLOR_BGR2RGB)
    
    # Get dimensions of original to prevent cv2.error: addWeighted shape mismatch
    ...
    # FIX: Resize heatmap to match the original source exactly
...
if not st.session_state["logged_in"]:
    ...
    if files:
        ...
        if st.button("INITIATE DEEP SCAN"):
            ...
            st.download_button("📥 EXPORT DOSSIER (.ZIP)", zip_buffer.getvalue(), f"CASE_{case_id}.zip")
