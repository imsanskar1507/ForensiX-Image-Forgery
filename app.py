...
# New State to track analysis for the Report Section
...
if not st.session_state["logged_in"]:
            ...
            with st.form("forgot_gate"):
                ...
                if st.form_submit_button("RESET ACCESS KEY", use_container_width=True):
                    ...
                    if c.fetchone():
                        ...
                        conn.commit(); st.success("Updated!"); st.session_state["auth_mode"] = "login"; st.rerun()
                    else: st.error("Mismatch."); conn.close()
            if st.button("Back to Login"): st.session_state["auth_mode"] = "login"; st.rerun()
    ...
    if files:
        ...
        if st.button("INITIATE DEEP SCAN"):
            ...
            # Store results in state to show the report section
            st.session_state["analysis_done"] = True
            st.session_state["report_data"] = results
            st.session_state["zip_buffer"] = zip_out.getvalue()

    # --- NEW REPORT SECTION (Only visible after analysis) ---
    if st.session_state["analysis_done"]:
        ...
        with st.container():
            ...
            total = len(st.session_state["report_data"])
            forgeries = sum(1 for r in st.session_state["report_data"] if "FORGERY" in r["VERDICT"])
            m1.metric("Exhibits Scanned", total)
            m2.metric("Forgeries Identified", forgeries)
            m3.metric("System Confidence", "92.4%")
            
            # Data Table
            st.table(pd.DataFrame(st.session_state["report_data"]))
            
            # Download Button
            st.download_button(
                label="📥 DOWNLOAD OFFICIAL CASE DOSSIER (.ZIP)",
